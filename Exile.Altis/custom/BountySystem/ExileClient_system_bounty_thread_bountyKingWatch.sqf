...
if (player getVariable ["ExileBountyKing",false]) then
{
	...
	if (((getPosATL player) select 2) < _bountyMaxHeight) then
	{
		if(_isInTrader) then
		{
			if (_traderTime <= 0) then
			{
				...
				["failBountyKing", [0]] call ExileClient_system_network_send;
		...
		if(_isInTerritory && (((getPosATL player) select 2) <= 75)) then
		{
			if (_territoryTime <= 0) then
			{
				...
				player setVariable ["ExileBountyTerritory", 30, true];
				player setVariable ["ExileBountyHeight", 30, true];
				["failBountyKing", [1]] call ExileClient_system_network_send;
	...
	else
	{
		if (_heightTime <= 0) then
		{
			...
			player setVariable ["ExileBountyTerritory", 30, true];
			player setVariable ["ExileBountyHeight", 30, true];
			["failBountyKing", [2]] call ExileClient_system_network_send;
...
{
	diag_log format["LOCAL BOUNTYKINGWATCH STOPPED"];
};